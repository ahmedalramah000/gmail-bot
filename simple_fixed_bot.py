        ...
        if query.data == "get_chatgpt_code":
                ...
                try:
                    ...
                    try:
                        await context.bot.send_message(
                            ...
                            text="لم يتم العثور على الرسالة أو لا يمكن تعديلها. إليك رسالة جديدة."
                        )
                    except Exception as e2:
                        logger.error(f"فشل في إرسال رسالة جديدة: {e2}") except telegram.error.BadRequest as e:                if "There is no text in the message to edit" in str(e) or "Message to edit not found" in str(e) or "Message can't be edited" in str(e):                    # تم حذف الرسالة أو غير قابلة للتعديل                    try:                        await context.bot.send_message(                            chat_id=query.message.chat_id,                            text="🔍 جاري البحث عن آخر كود... انتظر قليلاً"                        )                    except Exception as e2:                        logger.error(f"فشل في إرسال رسالة جديدة: {e2}")                        return                else:                    logger.error(f"خطأ في API تيليجرام: {e}")                    return                        # البحث عن الكود (مع تجاهل التحقق من وجود ملف بيانات الاعتماد)            code_info = self.get_latest_verification_code(user_id)                        keyboard = [                [InlineKeyboardButton("🔄 تحديث", callback_data="get_chatgpt_code")],                [InlineKeyboardButton("🎬 شاهد شرح طريقة الدخول", callback_data="show_tutorial")]            ]            reply_markup = InlineKeyboardMarkup(keyboard)                        # تعديل الشرط لتجاهل رسالة خطأ ملف بيانات الاعتماد            if code_info:                if "error" in code_info and code_info["error"] == "rate_limit":                    keyboard_rate_limit = [                        [InlineKeyboardButton("🎬 شاهد شرح طريقة الدخول", callback_data="show_tutorial")]                    ]                    reply_markup_rate_limit = InlineKeyboardMarkup(keyboard_rate_limit)                                        try:
                        try:
                            await query.edit_message_text(                            "⚠️ لقد تجاوزت الحد الأقصى من الطلبات. يرجى المحاولة لاحقًا.\n\n"                            f"📧 البريد: <code>{TARGET_EMAIL}</code>\n"                            f"🔒 كلمة المرور: <code>{PASSWORD}</code>\n\n"                            f"<b>📝 طريقة الدخول:</b>\n"                            f'1. اضغط على "try another method" من الأسفل\n'                            f'2. اختر البريد الإلكتروني (الخيار الثالث)
                        except telegram.error.BadRequest as e:
                            logger.error(f"فشل في تعديل الرسالة: {e}")
                            try:
                                await context.bot.send_message(
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.error(f"فشل في إرسال رسالة جديدة: {e2}") except telegram.error.BadRequest as e:                    logger.error(f"فشل في تعديل رسالة الكود: {e}")                    try:                        await context.bot.send_message(                            chat_id=query.message.chat_id,                            text=message,                            reply_markup=reply_markup,                            parse_mode='HTML'                        )                    except Exception as e2:                        logger.error(f"فشل في إرسال رسالة الكود جديدة: {e2}")            else:                # رسالة محسنة عند عدم وجود كود                keyboard_no_code = [                    [InlineKeyboardButton("🔄 تحديث", callback_data="get_chatgpt_code")],                    [InlineKeyboardButton("🎬 شاهد شرح طريقة الدخول", callback_data="show_tutorial")]                ]                reply_markup_no_code = InlineKeyboardMarkup(keyboard_no_code)                                try:
                    try:
                        await query.edit_message_text(                        f"❌ لم يتم العثور على كود تحقق\nحاول مره اخري\n\n"                        f"📧 <b>بيانات تسجيل الدخول:</b>\n"                        f"البريد: <code>{TARGET_EMAIL}</code>\n"                        f"الباسورد: <code>{PASSWORD}</code>\n\n"                        f"<b>📝 طريقة الدخول:</b>\n"                        f'1. اضغط على "try another method" من الأسفل\n'                        f'2. اختر البريد الإلكتروني (الخيار الثالث)
                    except telegram.error.BadRequest as e:
                        logger.error(f"فشل في تعديل الرسالة: {e}")
                        try:
                            await context.bot.send_message(
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"فشل في إرسال رسالة جديدة: {e2}")elif query.data == "show_password":
            # إزالة التحقق من المسؤول للسماح لأي مستخدم بعرض كلمة المرور
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"فشل في إرسال رسالة جديدة: {e2}")elif query.data == "admin_panel":
            # التحقق من أن المستخدم هو المسؤول
            user_id = str(update.effective_user.id)
            if ADMIN_CHAT_ID and user_id != ADMIN_CHAT_ID:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"فشل في إرسال رسالة جديدة: {e2}")elif query.data == "back_to_main":
            ...
            if TUTORIAL_VIDEO_FILE_ID:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(f"فشل في إرسال رسالة جديدة: {e2}")# معالجة أزرار لوحة تحكم المسؤول
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"فشل في إرسال رسالة جديدة: {e2}")elif query.data == "admin_show_video":
            # التحقق من أن المستخدم هو المسؤول
            user_id = str(update.effective_user.id)
            if ADMIN_CHAT_ID and user_id != ADMIN_CHAT_ID:
                await query.answer("⛔ هذا الزر متاح فقط للمسؤول", show_alert=True)
            ...
            if not TUTORIAL_VIDEO_FILE_ID:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(f"فشل في إرسال رسالة جديدة: {e2}")return
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"فشل في إرسال رسالة جديدة: {e2}")# إرسال الفيديو
            await context.bot.send_video(
                chat_id=update.effective_chat.id,
                video=TUTORIAL_VIDEO_FILE_ID,
                caption=(
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"فشل في إرسال رسالة جديدة: {e2}")elif query.data == "confirm_delete_video":
            # التحقق من أن المستخدم هو المسؤول
            user_id = str(update.effective_user.id)
            if ADMIN_CHAT_ID and user_id != ADMIN_CHAT_ID:
                await query.answer("⛔ هذا الزر متاح فقط للمسؤول", show_alert=True)
            ...
            if success:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(f"فشل في إرسال رسالة جديدة: {e2}")else:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(f"فشل في إرسال رسالة جديدة: {e2}")elif query.data == "return_to_admin_panel":
            # التحقق من أن المستخدم هو المسؤول
            user_id = str(update.effective_user.id)
            if ADMIN_CHAT_ID and user_id != ADMIN_CHAT_ID:
                await query.answer("⛔ هذا الزر متاح فقط للمسؤول", show_alert=True)
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"فشل في إرسال رسالة جديدة: {e2}")def main():
    """تشغيل البوت."""
